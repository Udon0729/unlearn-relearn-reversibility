...
class UnlearningMethod(ABC):
    ...
    def __init__(
        ...
        # Per-step response masks set by the pipeline before compute_loss
        self._forget_rm: Tensor | None = None
        self._retain_rm: Tensor | None = None

    def set_forget_response_mask(self, rm: Tensor | None) -> None:
        self._forget_rm = rm

    def set_retain_response_mask(self, rm: Tensor | None) -> None:
        self._retain_rm = rm
